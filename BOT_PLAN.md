...
- [ ] Add surveyed road/handoff waypoints for Ironforge -> Dun Morogh -> Loch Modan -> Wetlands -> Menethil; the first `grand-expedition-v0` dry run stalls when asking for Ironforge -> Menethil as one giant leg.
  ...
  - [ ] Add an efficiency calculation for death recovery: compare ghost-walk-to-corpse-and-reclaim (free, but gated by a real server-side reclaim delay - observed 72-100s per death tonight, scaling with repeated deaths) against spirit-healer resurrection (instant, but costs durability + resurrection sickness, and still requires walking - alive, exposed to mobs - from the graveyard to the nearest path point, which is roughly the same distance as the corpse-walk since both start from the same graveyard spawn). Pick whichever is faster given current corpse distance, current reclaim delay, and distance from the graveyard to the nearest remaining waypoint/destination. Apply to both `recover-death` (regular route-goto transport) and the survey-replay resume logic.
