...
happening on the main feature branch - don't let it get stuck waiting for a
...
3. Apply *only* the shared-code hunks - if the same commit on the main
   branch also touched a headless-only call site (e.g. a new HTTP endpoint
   in `tools/headless_client/main.cpp` that exercises the fix), leave that
   part out. Check `git diff master feat/headless-bot-fleet -- <file>` first;
